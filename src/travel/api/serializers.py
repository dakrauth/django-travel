...
class TravelEntitySerializer(serializers.ModelSerializer):
    ...
    country_flag_emoji = serializers.ReadOnlyField(
        source="country.flag.emoji", allow_null=True
    )
    country_flag_svg = serializers.ImageField(
        source="country.flag.svg", allow_null=True
    )
...
class TravelUserLogSerializer(serializers.ModelSerializer):
    ...
    def get_logs(self, obj):
        return TravelLogSerializer(TravelLog.objects.filter(user=obj), many=True).data

    def get_entities(self, obj):
        return TravelEntitySerializer(
            TravelEntity.objects.filter(travellog__user=obj)
            .distinct()
            .select_related("country", "flag", "country__flag", "type"),
            many=True,
        ).data
...
def flag_data():
    return {
        "countries": FlagEntitySerializer(
            TravelEntity.objects.countries()
            .filter(code__in={*chain(*games.FLAG_GROUPS)})
            .select_related("flag"),
            many=True,
        ).data,
